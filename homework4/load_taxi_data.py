...
BUCKET_NAME = "data-engineering-zoomcamp-hw4-2026"

# If you authenticated through the GCP SDK you can comment out these two lines
CREDENTIALS_FILE = "gcs.json"
# client = storage.Client.from_service_account_json(CREDENTIALS_FILE)
# If commented initialize client with the following
client = storage.Client(project='data-engineering-zoomcamp-hw3')
...
YEAR_MONTH_TAXIS = [(year, month, taxi) for year in YEARS for month in MONTHS for taxi in TAXIS]
...
def download_file(year, month, taxi):
...
# Small wrapper to unpack the tuple
def download_file_wrapper(args):
    year, month, taxi = args
    return download_file(year, month, taxi)

if __name__ == "__main__":
    create_bucket(BUCKET_NAME)

    with ThreadPoolExecutor(max_workers=4) as executor:
        file_paths = list(executor.map(download_file_wrapper, YEAR_MONTH_TAXIS))
